...
int main(){
  ...
  f_halo_data.open("reduced_cosmo_pairs.txt");
  ...
  for(k=0; k < N_PAIRS; k++){
    ...
    for(i = 0; sph.theta <= double(PI); i++){
      ...
      for( j = 0; sph.phi < double(2*PI) and points>0; j++){
        sph.phi = sph.phi + (double(PI*2.0))/points; // Range for phi is 0 to 2pi and the number of points are detirmined above
	    ...
	    sep_prob = probability("gaussian", b_sep.up - ((b_sep.up - b_sep.low)/2.), ((b_sep.up - b_sep.low)/4.), obssep);
